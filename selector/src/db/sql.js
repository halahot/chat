export const sql_check_token = "SELECT registrator.check_token($1)"
export const sql_get_users = "SELECT u.login FROM registrator.users u JOIN taskworker.friend f ON f.friend_id = u.id WHERE (SELECT id FROM registrator.users WHERE token = $1) = f.user_id";
